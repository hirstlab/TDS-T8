...
Enforce the tests-first rule in CI: any change to application source (t8_daq_system/)
...
touching application code under `t8_daq_system/` also touch tests under `tests/`.
...
def categorize_files(
    ...
    for f in files:
        ...
        if not norm:
            continue
        if norm.startswith("t8_daq_system/"):
...
def evaluate_tests_first(
    ...
    if not src_files:
        msg = (
            "OK: No application source files ('t8_daq_system/') modified. "
    ...
    file_list = "\n  - ".join(src_files)
    msg = (
        "FAIL: Application source files modified under 't8_daq_system/' without corresponding "
