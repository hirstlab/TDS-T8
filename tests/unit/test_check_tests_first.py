"""
tests/unit/test_check_tests_first.py
Test suite for the tests-first CI gate in scripts/check_tests_first.py.

Verifies:
1. Changes touching t8_daq_system/ without tests/ fail.
2. Changes touching t8_daq_system/ AND tests/ pass.
...
def test_normalize_path():
    assert normalize_path("t8_daq_system\\gui\\app.py") == "t8_daq_system/gui/app.py"
...
def test_categorize_files():
    files = [
        "t8_daq_system/config/theme.py",
        "t8_daq_system/state/beamline.py",
        "tests/test_theme.py",
        "docs/adr/0001.md",
    ...
    assert srcs == ["t8_daq_system/config/theme.py", "t8_daq_system/state/beamline.py"]
...
def test_no_src_changes_passes():
    ...
    assert "No application source files ('t8_daq_system/') modified" in msg
...
def test_src_and_test_changes_passes():
    files = [
        "t8_daq_system/hardware/raster_model.py",
...
def test_src_changes_without_test_fails():
    ...
    assert (
        "FAIL: Application source files modified under 't8_daq_system/' without corresponding test changes "
        "under 'tests/'"
    ) in msg
    assert "t8_daq_system/hardware/raster_model.py" in msg


def test_escape_via_explicit_reason():
    files = ["t8_daq_system/gui/widgets/inputs.py"]
...
def test_cli_end_to_end_scenarios(tmp_path, monkeypatch, capsys):
    ...
    code = main(["--files", "t8_daq_system/state/beamline.py"])
    assert code == 1

    # 3. Src changes with test changes -> exit 0
    code = main(["--files", "t8_daq_system/state/beamline.py", "tests/test_beamline.py"])
    assert code == 0

    # 4. Src changes with explicit exempt reason -> exit 0
    code = main(["--files", "t8_daq_system/state/beamline.py", "--exempt-reason", "Refactor only"])
    assert code == 0

    # 5. Src changes with message annotation tag -> exit 0
    code = main([
        "--files", "t8_daq_system/state/beamline.py",
